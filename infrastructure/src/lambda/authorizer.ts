...
const getToken = (event: APIGatewayAuthorizerEvent) => {
  ...
  if (event.type !== "TOKEN")
    throw new Error(
      `event.type parameter must have value TOKEN , but actual value is ${event.type}`
    );
  ...
  if (!token)
    throw new Error(
      "event.authorizationToken parameter must be set, but got null"
    );
...
const verifyToken = async (token: string): Promise<string | jwt.JwtPayload> => {
  ...
  try {
    ...
    const signingKey =
      ...
      (key as jwks.RsaSigningKey).rsaPublicKey; // 公開鍵を取得
